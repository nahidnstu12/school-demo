...
import { createTeacher, getTeacherById, getTeacherDesignations, updateTeacher } from "@/backend/actions/teacher.action";
...
export function TeacherForm({ 
  ...
  useEffect(() => {
    if (teacherData && Object.keys(teacherData).length > 0) {
      methods.reset({
        ...
        joiningDate: teacherData.joiningDate ? new Date(teacherData.joiningDate).toISOString().split('T')[0] : null,
  ...
  const designationOptions = options?.designations?.map((designation) => ({
    id: designation as string,
    value: designation as string,
    label: designation as string
  })) || [];
